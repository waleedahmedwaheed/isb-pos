...
//echo $_SESSION["s_id"]."asdasdasdasdas";exit;
	...
	if(isset($_GET["pp_id"]))
	{
		...
		$getSQL = "select * from production_prod where pp_id = '".$_GET["pp_id"]."' and ppr_id = '".$_GET["ppr_id"]."' and pp_status = 0";
...
<script>
var ppr_id = <?php echo $_GET["ppr_id"]; ?>;
$("#dttable").load("process_prod.php?ppr_id="+ppr_id);
$("select#cat").change(function(){
	...
	if (cat.length > 0 ) { 
	 ...
	 $.ajax({
			type: "POST",
			url: "fetch-recordstp.php",
			data: "cat="+cat+"&ppr_id="+ppr_id,
			cache: false,
			beforeSend: function () { 
				$('#product').html('<img src="loader.gif" alt="" width="24" height="24">');
			},
			success: function(html) {    
				$("#product").load("fetch-recordstp.php?cat="+cat+"&ppr_id="+ppr_id);
				$("#dttable").load("process_prod.php?ppr_id="+ppr_id);
			}
		});
	} 
